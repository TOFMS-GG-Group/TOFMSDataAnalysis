...
#     numIons = 100000
#
#     return calculate_critical_value(alpha, numIons, "/Users/tylerjaacks/Desktop/SIS-Hist.csv")
...
def calculate_critical_value(alpha, num_ions, filename):
    ...
    with np.nditer(cmpd_array, flags=['multi_index'], op_flags=['readwrite']) as it:
        for x in it:
            print("Doing Monte Carlo Simulations!")
...
def main(argv):
    calculate_critical_value(argv[0], argv[1], argv[2])
